    ...
    def fill_attendance():
        ...
        if subb=='':
            ...
            def create_csv():
                ...
                csv_name='C:\\Users\\panda\\OneDrive\\Desktop\\Attendace_management_system-master\\Attendace_management_system-master\\Attendance\\Manually Attendance/'+DB_table_name+'.csv'
            ...
            def attf():
                import subprocess
                subprocess.Popen('explorer /select,"C:\\Users\\panda\\OneDrive\\Desktop\\Attendace_management_system-master\\Attendace_management_system-master\\Attendance\\Manually Attendance\\-------Check atttendance-------"')
...
def take_img():
    ...
    if l1 == '':
        ...
        try:
            ...
            if detector.empty():
                print("Error: Unable to load haarcascade_frontalface_default.xml. Please check the file path.")
                return
...
def subjectchoose():
    def Fillattendances():
        ...
        # Use the relative path for the haarcascade file
        ...
        df = pd.read_csv("StudentDetails\\StudentDetails.csv", header=None, names=['Enrollment', 'Name', 'Date', 'Time'])
        cam = cv2.VideoCapture(0)
        ...
        while True:
            ...
            for (x, y, w, h) in faces:
                ...
                if conf < 70:
                    ...
                    if len(aa) > 0:
                        aa = aa[0]  # Take the first name if multiple
                    else:
                        aa = 'Unknown'
                    tt = str(Id) + "-" + str(aa)
        ...
        try:
            ...
            for index, row in attendance.iterrows():
                insert_data = "INSERT INTO " + DB_Table_name + " (ID,ENROLLMENT,NAME,DATE,TIME) VALUES (0, %s, %s, %s,%s)"
                VALUES = (str(row['Enrollment']), str(row['Name']), str(row['Date']), str(row['Time']))
    ...
    def Attf():
        import subprocess
        subprocess.Popen('explorer /select,"C:\\Users\\panda\\OneDrive\\Desktop\\Attendace_management_system-master\\Attendace_management_system-master\\Attendance\\-------Check atttendance-------"')
    ...
    def log_in():
        ...
        if username == 'kushal' :
            if password == 'kushal14320':
                ...
                cs = 'C:\\Users\\panda\\OneDrive\\Desktop\\Attendace_management_system-master\\Attendace_management_system-master\\StudentDetails\\StudentDetails.csv'
                with open(cs, newline="") as file:
                    reader = csv.reader(file)
                    r = 0
...
def trainimg():
    ...
    # Ensure the TrainingImage directory exists
    if not os.path.exists('TrainingImage'):
        print("TrainingImage directory not found. Please add images and try again.")
        return

    try:
        faces, Id = getImagesAndLabels('TrainingImage')
        if len(faces) == 0:
            print("No images found in TrainingImage folder. Please add images and try again.")
            return
    except Exception as e:
